...
const int MAX_N = 100; // Максимальное количество столбцов (1 шифр + характеристики)
...
void deleteRow(int matrix[MAX_M][MAX_N], int& m, int n, int rowIndex) {
...
int main() {
    ...
    bool toDelete[MAX_M] = { false }; // Массив флагов для удаления
    ...
    printMatrix(matrix, m, n, "Оставшаяся после резекции матрица");
